@extends('admin_layout')
@section('title', 'Cửa hàng bán quần áo thời trang Heaven | Danh sách khách hàng')

@section('admin_content')
    @include('admin.theme.sidebar.customer')

    {!! Toastr::message() !!}

    <div id="main">
        <div class="row">
            <div id="breadcrumbs-wrapper" data-image="{!! asset('public/backend/app-assets/images/gallery/breadcrumb-bg.jpg') !!}">
                <div class="container">
                    <div class="row">
                        <div class="col s12 m6 l6">
                            <h5 class="mt-0 mb-0 breadcrumbs-title">
                                <span>Khách hàng</span>
                            </h5>
                        </div>
                        <div class="col s12 m6 l6 right-align-md">
                            <ol class="mb-0 breadcrumbs">
                                <li class="breadcrumb-item">
                                    <a href="{!! route('dashboard') !!}">
                                        <i class="fa fa-home"></i>
                                    </a>
                                </li>
                                <li class="breadcrumb-item active">
                                    Danh sách khách hàng
                                </li>
                            </ol>
                        </div>
                    </div>
                </div>
            </div>

            <div class="col s12">
                <div class="container">
                    <div class="section section-data-tables">
                        <div class="row">
                            <div class="col s12">
                                <div class="card">
                                    <div class="card-content">
                                        <div class="row">
                                            <div class="col s12">
                                                <table id="page-length-option" class="display">
                                                    <thead>
                                                        <tr>
                                                            <th>#</th>
                                                            <th>Tên khách hàng</th>
                                                            <th>Hình ảnh</th>
                                                            <th>Trạng thái</th>
                                                            <th style="width: 160px;">Hành động</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        @php
                                                            $stt = 1;
                                                        @endphp
                                                        @foreach ($customers as $customer)
                                                            <tr>
                                                                <td class="pro-list-img">
                                                                    {{ $stt++ }}
                                                                </td>

                                                                <td class="pro-name">
                                                                    <span>{{ $customer->customer_name }}</span>
                                                                </td>

                                                                <td class="pro-list-img">
                                                                    @if ($customer->customer_social == 0)
                                                                        <img src="{{URL::to('public/uploads/customer/'.$customer->customer_image)}}" height="60" width="60" alt="{{ $customer->customer_name }}"/>
                                                                    @else
                                                                        <img src="{{ $customer->customer_image }}" height="60" width="60" alt="{{ $customer->customer_name }}"/>
                                                                    @endif
                                                                </td>

                                                                <td>
                                                                    <?php if ($customer->customer_status == 1) { ?>
                                                                        <a href="{{ route('customer.unactive_customer', ['customer_id' => $customer->customer_id]) }}" class="green-text">
                                                                            <span class="chip green lighten-5">
                                                                                Hoạt động
                                                                            </span>
                                                                        </a>
                                                                    <?php } else { ?>
                                                                        <a href="{{ route('customer.active_customer', ['customer_id' => $customer->customer_id]) }}" class="orange-text">
                                                                            <span class="chip red lighten-5">
                                                                                Bị khóa
                                                                            </span>
                                                                        </a>
                                                                    <?php } ?>
                                                                </td>

                                                                <td class="action-icon">
                                                                    <a class="btn btn-grd-info btn-sm"
                                                                        href="{{ route('customer.view_customer', ['customer_id' => $customer->customer_id]) }}">
                                                                        <i class="fa fa-eye" style="font-size: 16px" aria-hidden="true"></i>
                                                                    </a>

                                                                    @if ($customer->customer_social == 0)
                                                                        <form style="display: inline-block;">
                                                                            @csrf
                                                                            <button type="button"
                                                                                class="btn btn-grd-danger btn-sm"
                                                                                style="outline: none"
                                                                                onclick="deleteCustomer({{ $customer->customer_id }})"><i
                                                                                    class="fa fa-trash-o fa-fw"></i></button>
                                                                        </form>
                                                                    @else
                                                                        <button type="button"
                                                                            class="btn btn-grd-danger btn-sm"
                                                                            style="outline: none"
                                                                            onclick="return alert('Tài khoản đăng ký bởi MXH không thể xóa!')"><i
                                                                                class="fa fa-trash-o fa-fw"></i></button>
                                                                    @endif

                                                                    <a class="btn btn-grd-warning btn-sm"
                                                                        href="{{ route('customer.history_order', ['customer_id' => $customer->customer_id]) }}">
                                                                        <i class="fa fa-history" style="font-size: 16px"></i>
                                                                    </a>
                                                                </td>
                                                            </tr>
                                                        @endforeach
                                                    </tbody>
                                                    <tfoot>
                                                        <tr>
                                                            <th>#</th>
                                                            <th>Tên khách hàng</th>
                                                            <th>Hình ảnh</th>
                                                            <th>Trạng thái</th>
                                                            <th style="width: 160px;">Hành động</th>
                                                        </tr>
                                                    </tfoot>
                                                </table>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <!-- START RIGHT SIDEBAR NAV -->

                    {{-- @include('admin.theme.rightsidebar') --}}

                    <!-- END RIGHT SIDEBAR NAV -->
                    {{-- @include('admin.theme.bottomsidebar') --}}
                </div>
                <div class="content-overlay"></div>
            </div>

        </div>
    </div>
@endsection

@push('scripts')

    <script type="text/javascript">
        $(document).ready(function() {
            $('#page-length-option').DataTable({
                "processing": true,
                "oLanguage": {
                    "sSearch": '<span class="fs-14">Tỉm kiếm: </span> ',
                    "sZeroRecords": "Không có dữ liệu nào trong bảng",
                    "sLengthMenu": '<span class="fs-14">Hiển thị</span> <select style="display: none">' +
                        '<option value="10">10</option>' +
                        '<option value="20">20</option>' +
                        '<option value="30">30</option>' +
                        '<option value="40">40</option>' +
                        '<option value="50">50</option>' +
                        '<option value="-1">Tất cả</option>' +
                        '</select> <span class="fs-14">bản ghi</span>',
                    "sInfo": "Hiển thị _START_ đến _END_ trong tổng số bản ghi là _TOTAL_",
                    "sProcessing": 'Loading <i class="fa fa-spinner" style="transition: 2s;"></i>',
                    "oPaginate": {
                        "sNext": "Trang sau",
                        "sPrevious": "Trang trước",
                    }
                },
            });
        });
    </script>

    <!-- Xóa khách hàng -->
    <script>
        function deleteCustomer(customer_id) {
            var _token = $('input[name="_token"]').val();

            swal({
                title: "Bạn có chắc xóa không?",
                text: "Bạn sẽ không thể khôi phục lại tệp này!",
                type: "warning",
                showCancelButton: true,
                cancelButtonText: "Huỷ",
                confirmButtonColor: "#DD6B55",
                confirmButtonText: "Vâng, xóa nó!",
                closeOnConfirm: false
            }, function(isConfirm) {
                if (!isConfirm) return;
                $.ajax({
                    url: '{{ route('customer.delete') }}',
                    method: 'POST',
                    data: {
                        customer_id: customer_id,
                        _token: _token
                    },
                    dataType: "html",
                    success: function() {
                        setTimeout(function() {
                            location.reload();
                        }, 1000)
                    },
                });
            });
        }
    </script>
@endpush
